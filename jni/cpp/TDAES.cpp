 ...
 *
 */
std::string TDAES::encodeAES( const std::string& password, const std::string& data )
{
    AES_KEY aes_key;
...
std::string TDAES::decodeAES( const std::string& strPassword, const std::string& strData )
{
    AES_KEY aes_key;
    if(AES_set_decrypt_key((const unsigned char*)strPassword.c_str(), 256, &aes_key) < 0)
    {
        return "null";
    }
    std::string strRet;
