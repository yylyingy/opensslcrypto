...
//    r = PEM_read_bio_RSAPrivateKey(bio, NULL, NULL, NULL);
    ...
    char *de = (char *)malloc((size_t) (flen + 1));
    memset(de, 0, (size_t) (flen + 1));

    const char *dataP=data.c_str();


    ret =RSA_private_decrypt(flen,(const unsigned char *)dataP, ( unsigned char *)de, r,
...
std::string TDRSA::encryptRSA(const std::string& data, int *lenreturn) {
